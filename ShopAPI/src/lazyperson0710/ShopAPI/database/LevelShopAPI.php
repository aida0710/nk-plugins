...
class LevelShopAPI {
	...
	public function init() : void {
		...
		$this->register(VanillaBlocks::SHULKER_BOX()->asItem(), 3000, 1, 50, 'シュルカーボックス'); ## Shulker_Box
		...
		$this->register(VanillaBlocks::RAIL()->asItem(), 300, 1, 50, 'レール ', false); ## Rail
		$this->register(VanillaBlocks::POWERED_RAIL()->asItem(), 300, 1, 50, '加速レール ', false); ## POWERED_RAIL
		$this->register(VanillaBlocks::ACTIVATOR_RAIL()->asItem(), 300, 1, 50, '感知レール ', false); ## ACTIVATOR_RAIL
		$this->register(VanillaBlocks::DETECTOR_RAIL()->asItem(), 300, 1, 50, 'アクティベーター レール ', false); ## DETECTOR_RAIL
		/*Shop4*/
		#Elytra
		...
		$this->register(VanillaBlocks::BEACON()->asItem(), 300000, 1, 180, 'ビーコン ', false); ## Beacon
		#Heads
		...
		$this->register(VanillaBlocks::FLETCHING_TABLE()->asItem(), 25000, 1, 250, '矢細工台 ', false); ## Fletching_Table
		$this->register(VanillaBlocks::COMPOUND_CREATOR()->asItem(), 25000, 1, 250, '化合物作成機 ', false); ## Compound_Creator
		$this->register(VanillaBlocks::LOOM()->asItem(), 25000, 1, 250, '織機 ', false); ## Loom
		$this->register(VanillaBlocks::ELEMENT_CONSTRUCTOR()->asItem(), 25000, 1, 250, '元素構成機 ', false); ## Element_Constructor
		$this->register(VanillaBlocks::LAB_TABLE()->asItem(), 125000, 1, 250, '実験テーブル ', false); ## Lab_Table
		$this->register(VanillaBlocks::MATERIAL_REDUCER()->asItem(), 25000, 1, 250, '物質還元器 ', false); ## Material_Reducer
		$this->register(VanillaBlocks::BREWING_STAND()->asItem(), 25000, 1, 250, '調合台 ', false); ## Brewing_Stand
		$this->register(VanillaBlocks::ENCHANTING_TABLE()->asItem(), 25000, 1, 250, 'エンチャントテーブル ', false); ## Enchanting_Table
		...
		$this->register(VanillaBlocks::TNT()->asItem(), 25000, 1, 250, 'TNTブロック ', false); ## TNT
		$this->registerFromId(-239, 0, 10, 1, 250, 'ターゲット ', false); ## Target
		$this->register(VanillaBlocks::TRIPWIRE_HOOK()->asItem(), 25000, 1, 250, 'トリップワイヤーフック ', false); ## Tripwire_Hook
		$this->register(VanillaBlocks::TRAPPED_CHEST()->asItem(), 2500, 1, 250, 'トラップチェスト ', false); ## trap_chest
		$this->register(VanillaBlocks::REDSTONE_TORCH()->asItem(), 2500, 1, 250, 'レッドストーントーチ ', false); ## Redstone_torch
		$this->register(VanillaBlocks::REDSTONE_REPEATER()->asItem(), 2500, 1, 250, 'リピーター ', false); ## Redstone_Repeater
		$this->register(VanillaBlocks::REDSTONE_COMPARATOR()->asItem(), 2500, 1, 250, 'コンパレーター ', false); ## Redstone_Comparator
	}
