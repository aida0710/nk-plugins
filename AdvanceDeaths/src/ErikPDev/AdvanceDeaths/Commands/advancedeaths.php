...
class advancedeaths {
    ...
    public function __construct($plugin, $database) {
        $this->plugin = $plugin;
    ...
    public function onCommand(CommandSender $player, Command $cmd, string $label, array $args): bool {
        ...
        if (!$player instanceof Player) {
            $player->sendMessage("§bAdvance§cDeaths §6>§c Please run the command from in-game.");
            return false;
        }
        $form = new CustomForm(function (\pocketmine\player\Player $player, $data) use (&$PlayerNames) {
            if (!isset($data[2])) {
                return $player->sendMessage("§bAdvance§cDeaths §6>§r No Player Name has been selected.");
            }
            if ($data[2] > count($PlayerNames)) {
                return $player->kick("             §b~ Advance§cDeaths ~\nKicked for attempted data manipulation.", false);
            }
