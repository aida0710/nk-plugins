...
class GachaItemAPI {
     ...
     * @return void
     */
    private function dischargeItemRegister(string $category, RarityMap $rank, Item $item, int $count, string $name, string $lore, array $enchants, array $level, array $nbt): void {
        $rank = match ($rank) {
            RarityMap::C() => "C",
            RarityMap::UC() => "UC",
            RarityMap::R() => "R",
            ...
            RarityMap::SSR() => "SSR",
            RarityMap::L() => "L",
            default => throw new \Error("Gacha : アイテム登録時にランクに登録されていないレアリティーが入力された為プラグインを停止します"),
        };
        $this->checkFunctionArgument($category, $enchants, $level);
     ...
     * @return void
     */
    private function checkFunctionArgument(string $category, array $enchants, array $level): void {
