...
namespace ColinHDev\CSkull\libs\poggit\libasynql\generic;
...
class GenericVariable implements JsonSerializable {
     ...
     *
     * If the SQL dialect does not support empty list declarations <code>()</code>, and <code>list:</code> is used, an
     * exception will be thrown when an empty array is passed as the value. If <code>list?</code> is used, a
     * randomly-generated string will be filled into the array to satisfy the language's requirements. This might cause
     * undesired behaviour unless you are only using this variable for a simple <code>IN :list</code> condition.
     *
     * As this may expose a security breach or a performance degrade, plugins are not encouraged to use this method.
     * Instead it is more desirable to check if the array is empty before passing the value into libasynql.
     *
     * @return bool
     */
