...
class SkullEntity extends Human implements ChunkListener {
     ...
     * vanilla values, expanded by @link SkullEntity::INFLATE_GEOMETRY. This was done because otherwise, the skull
     * block under the entity would be partly visible, especially in those places where there is no second head layer.
     * So we increase the size of the geometry so that the skull block below is never visible and will always be hidden
     * underneath the entity. By setting @link SkullEntity::INFLATE_GEOMETRY to 0.3 and moving the geometry upwards
     * with the y origin value of
     * 0.125, we get the perfect result, where the entity's size is only slightly bigger than the hitbox of the skull
     * block.
     */
