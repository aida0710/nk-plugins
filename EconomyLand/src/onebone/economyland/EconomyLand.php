...
class EconomyLand extends PluginBase implements Listener {
	...
	public function onCommand(CommandSender $sender, Command $cmd, string $label, array $param) : bool {
		switch ($cmd->getName()) {
			case "s":
				...
				if (in_array($worldName, WorldCategory::LifeWorld, true) || in_array($worldName, WorldCategory::AgricultureWorld, true)) {
			...
			case "land":
				...
				switch ($sub) {
					case "buy":
						...
						if (in_array($sender->getWorld()->getFolderName(), $this->getConfig()->get("buying-disallowed-worlds", []), true)) {
					...
					case "give":
						...
						if ($sender->getName() !== $info["owner"] && !$sender->hasPermission("economyland.land.give.others")) {
					...
					case "kick":
						...
						if ($sender->hasPermission("economyland.command.land.invite.remove.others") && $info["owner"] !== $sender->getName()
							|| $info["owner"] === $sender->getName()) {
			...
			case "landsell":
				...
				switch ($id) {
					case "here":
						...
						if ($info["owner"] !== $sender->getName() && !$sender->hasPermission("economyland.landsell.others")) {
	...
	public function addLand($player, $startX, $startZ, $endX, $endZ, $level, $expires = null, &$id = null) {
		...
		if ($result !== false && $this->getConfig()->get("allow-overlap", false) === false) {
	...
	public function permissionCheck(Event $event) : bool {
		...
		if (in_array($level, $this->getConfig()->get("non-check-worlds", []), true)) {
			return false;
		}
		$info = $this->db->canTouch($x, $z, $level, $player);
		if ($info === -1) {
			if ($this->getConfig()->get("white-world-protection", [])) {
				if (in_array($level, $this->getConfig()->get("white-world-protection", []), true) && !$player->hasPermission("economyland.land.modify.whiteland")) {
