...
class XPCommands extends Command implements Listener {
    ...
    public function __construct(Main $main) {
        ...
        $this->plugin = $main;
        $this->setPermission($this->plugin->getConfig()->get("permission-command"));
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        if ($args[0]) {
            switch (strtolower($args[0])) {
                case 'add':
                    $amount = $args[2];
                    $target->getXpManager()->addXpLevels((int)$amount, true);
                ...
                case 'remove':
                    $amount = $args[2];
                    $target->getXpManager()->subtractXpLevels((int)$amount);
