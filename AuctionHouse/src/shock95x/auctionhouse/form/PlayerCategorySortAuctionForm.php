    ...
    public function handleSubmit(Player $player): void {
        if ($this->offlinePlayer->getValue() === "") {
            ...
            if ($value === "現在オンラインのプレイヤーは自身以外に存在しません") {
                SendMessage::Send($player, "現在オンラインのプレイヤーは自身以外に存在しない為、表示したいプレイヤーの名前をInputに直接入力してください", "Bazaar", false);
                return;
            }
        } else {
            $value = $this->offlinePlayer->getValue();
            if ($value === $player->getName()) {
                SendMessage::Send($player, "自身のアイテムを確認する場合は/ah shopから表示することが可能です", "Bazaar", false);
                return;
            }
        }
