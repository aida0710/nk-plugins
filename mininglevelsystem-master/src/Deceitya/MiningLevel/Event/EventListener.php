    ...
    public function getBreakEvent(Event $event, Player $player){
        ...
        for ($up = 0; $exp >= $upexp; $up++) {
            ...
            if (isset($this->config['item'][$level])) {
                ...
                $giveitem = "§bLevel §7>> §aレベルアップボーナスとして{$item}が付与されました";
                $player->sendMessage($giveitem);
            } else if ($level % 5 == 0) {
                $givemoney = "§bLevel §7>> §aレベルアップボーナスとして8000円が付与されました";
                EconomyAPI::getInstance()->addMoney($player, 8000);
                $player->sendMessage($givemoney);
            }
        }
            ...
            if ($level % 50 == 0) {
                ...
                switch ($level) {
                    ...
                    case 10:
                        ...
                        break;
                    case 15:
                        $player->sendMessage("§bTutorial §7>> §gインベントリの物を一気にストレージに入れたいときは/stall !!");
                        $player->sendMessage("§bTutorial §7>> §gオーバーワールド4&javaが解放されました！通常よりも鉱石の量が多いのが特徴です。ただし、javaのワールドはmininglevel経験値が取得できないため注意が必要です！");
                        break;
                    case 18:
                    ...
                    case 25:
                        ...
                        break;
                    case 30:
                        $player->sendMessage("§bTutorial §7>> §gサーバーオリジナルレシピが存在します！/recipeから見れるよ！");
                        $player->sendMessage("§bTutorial §7>> §gMiningWorldが解放されました！一週間に一度リセットされる特殊なワールドです！");
                        break;
                    case 45:
                        $player->sendMessage("§bTutorial §7>> §gショップ2が解放されました！/shopから確認してみよう！");
                        break;
                    case 50:
                        $player->sendMessage("§bTutorial §7>> §gLevelShop3が解放されました！/shop");
                        $player->sendMessage("§bTutorial §7>> §gNetheriteMiningToolsが解放されました！修繕可能なため範囲破壊をとことん楽しんでください！");
                        break;
                    case 80:
                        $player->sendMessage("§bTutorial §7>> §gShop4が解放されました！/shop");
                        $player->sendMessage("§bTutorial §7>> §gコマンドから道具の修繕が可能になりました！/repair");
                        break;
                    case 120:
                        $player->sendMessage("§bTutorial §7>> §gLevelShop5が解放されました！/shop");
                        break;
                    case 180:
